...
public class BJ3197_백조의호수{
    static LinkedList<NodeData> srch = new LinkedList<>(); // Breadth First Search를 위한 연결리스트
    static LinkedList<NodeData> h2o = new LinkedList<>(); // 물좌표에 해당하는 연결리스트
    static LinkedList<NodeData> swanLoc = new LinkedList<>(); // 백조 좌표 정보
    ...
    // 연결 확인
    private static boolean isConnect(LinkedList<NodeData> waitingQueue){
        //Iterator<NodeData> i = srch.listIterator();
                ...
                if (newRow >= 0 && newRow < r && newCol >= 0 && newCol < c){
                    if (!isVisited[newRow][newCol]){
                        ...
                        if (graph[newRow][newCol] == 'X'){
                            waitingQueue.add(new NodeData(newRow, newCol));
                        }
                        else{
                            srch.add(new NodeData(newRow, newCol));
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < sizeConstant; i++){
            ...
            for (int a = 0; a < dir.length; a++){
                ...
                if (newRow >= 0 && newRow < r && newCol >= 0 && newCol < c){
                    if (graph[newRow][newCol] == 'X'){
                        graph[newRow][newCol] = '.';
                        h2o.add(new NodeData(newRow, newCol));
                    }
                }
            }
        }
    ...
    public static void main(String[] args) throws IOException{
        ...
        for (int i = 0; i < r; i++){
            ...
            for (int j = 0; j < c; j++){
                ...
                if (graph[i][j] == 'L')
                    swanLoc.add(new NodeData(i, j));
                if (graph[i][j] != 'X')
                    h2o.add(new NodeData(i, j));
            }
        }
        ...
        outerLoop: while (true){
            LinkedList<NodeData> waitingQueue = new LinkedList<>();
            isConnect = BJ3197_백조의호수.isConnect(waitingQueue);
